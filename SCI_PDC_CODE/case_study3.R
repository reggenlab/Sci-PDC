setwd("C:/Users/smadh/OneDrive/Desktop/rank_study_data/age_based")
library(data.table)
library(FSA)
random=data.frame(fread('un_o_scores.csv',header = T,stringsAsFactors = F,check.names = F), row.names=1)

# cell depth unsatisfied
#data2=apply(random,2,function(x){perc(x,0.05,"lt")})

# disease specific filtering ###########
random2=random[1:1811,]
data_2=apply(random2,1, function(x){perc(x,0.2,"lt")})
...
beta2=as.matrix(rownames(dispath)[148:324])

file= read.csv('comb_corr_file.csv',header = T,row.names = 1,stringsAsFactors = F,check.names = F)
...
###############   pval based
adult_pval=read.table('comb_pval_file.csv',header = T,sep = ",",check.names = F,row.names = 1,stringsAsFactors = F,)
...
###############   pval based
adult_rank=read.table('comb_rank_file.csv',header = T,sep = ",",check.names = F,row.names = 1,stringsAsFactors = F,)
...
write.table(test2,'old_pathset.txt',row.names = F,sep='\t',quote=F)
...
pathset= read.table('old_pathset.txt',header = T,sep = "\t",check.names = F,stringsAsFactors = F,)
vec=c('Diabetes_mellitus',pathset$Row.names)

names.use <- rownames(random)[(rownames(random) %in% vec)]
adult_final <- random[ names.use,]

write.csv(t(adult_final),file='old_pathset_score.csv',quote = F)
############################################################
...
data<-read.csv('adult_pathset_score.csv',header = T,stringsAsFactors = F,check.names = F,row.names = 1)
...
########################################  NORMAL    ######################################################
data<-read.csv('old_pathset_score.csv',header = T,stringsAsFactors = F,check.names = F,row.names = 1)
...
data<-read.csv('young_pathset_score.csv',header = T,stringsAsFactors = F,check.names = F,row.names = 1)
...
cl = makeCluster(2)
# 
bay_main3 = boot.strength(as.data.frame(data3), R = 1001, algorithm = "hc",cluster = cl)
avg_main3 = averaged.network(bay_main3, threshold = 0.3)
...
plotGML(data1_fit, fn="adult_pgm.gml", weight=TRUE,vars=rownames(data3))
g1_file = read.graph(file="adult_pgm.gml", format="gml")
...
plotGML(data2_fit, fn="old_pgm.gml", weight=TRUE,vars=rownames(data3))
g2_file = read.graph(file="old_pgm.gml", format="gml")
...
data3_fit <- XMRF(as.matrix(data3),method="GGM",stability="STAR",nlams=15, N=1001,beta=0.3,parallel = T,nCpus=10)
#data1_fit<-XMRF(as.matrix(data3), method="PGM",lambda.path=lambda, sth=0.3,N=101,parallel = T,nCpus=10)

plotGML(data3_fit, fn="young_pgm.gml", weight=TRUE,vars=rownames(data3))
g3_file = read.graph(file="young_pgm.gml", format="gml")
...
wgt2=bay_main2[pos,]
write.table(wgt2,file="old_baystrength.txt",quote=F,row.names=F,sep="\t")
...
library("ggpubr")
options(scipen=999)
file=read.table('old_baystrength.txt',header = T,sep = "\t")
...
p=ggplot(file, aes(x=file$to, y=file$strength, color=jaccard)) +
  ...
  geom_point(size=6)+
  coord_flip() +
  ggtitle("Pathways edge weight in old pancreatic beta cells bayesian model")+
...
write.table(wgt2,file="old_mark_edges.txt",row.names = F,col.names = T,quote=F,sep="\t")
##################################
# 
# ####################
# #ig_df=structure
...
##################    COMMON PATHWAYS AGE
setwd("C:/Users/smadh/OneDrive/Desktop/rank_study_data/age_based/age_network")

com=read.table('age_common_bay.txt',header = T,sep='\t',check.names = F)
library(tidyverse)
library(ggplot2)
p=com %>%
  ggplot(aes(factor(PATHWAY), EDGE_STRENGTH, fill = GROUP)) + 
  ...
  ggtitle("Directly interacting pathways of diabetes mellitus (AGE-WISE GROUP)")+
  theme(plot.background=element_rect(fill="white"),
        plot.margin = unit(c(25, 25, 25, 25), "cm")) #top, right, bottom, left

p2=p +theme_classic()+ 
  theme(plot.title = element_text(size=25, face="bold", margin = margin(10, 0, 10, 0)))+
  theme(
    axis.title.x = element_text(color="black", vjust=-0.55,size = 24),
    axis.title.y = element_text(color="black" , vjust=0.55,size = 24)   
  )+
  theme(axis.text.x=element_text(color="black",size=12, vjust=0.5),
        axis.text.y=element_text(color="black",size=18, vjust=0.5))+
  labs(x='Pathways',y='Edge strength')+
  theme(legend.title = element_text(size=25, face="bold"))+
  #scale_color_discrete(name="Sample Type")+
  theme(legend.position="right",legend.text = element_text(size=15, face="bold"))
p2
